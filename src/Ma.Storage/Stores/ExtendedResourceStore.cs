...
public class ExtendedResourceStore : IExtendedResourceStore
{
    ...
    private async Task Update(ApiResource apiResource, ApiResource existingResource)
    {
        ...
        try
        {
            ...
            var toAddScopesFromApiResource =
                apiResource.Scopes.Where(scope =>
                    !existingResource.Scopes.Contains(scope, StringComparer.InvariantCultureIgnoreCase)).Distinct();
            var toRemoveScopesFromApiResource =
                existingResource.Scopes.Where(scope =>
                    !apiResource.Scopes.Contains(scope, StringComparer.InvariantCultureIgnoreCase)).Distinct();
            ...
            foreach (var toBeRemovedScopeId in toBeRemovedScopesIds)
            {
                ...
                thereWasChanges = true;
                if (await CheckIfAnyResourceHasThisScope(connection, transaction, toBeRemovedScopeId))
                {
                    continue;
                }
    ...
    {
        return connection.ExecuteScalarAsync<int>(@"INSERT INTO api_scopes(name) VALUES(@scope) RETURNING id",
            new {scope},
            transaction);
    }
    ...
    private static Task<int> InsertApiResource(IDbConnection connection, IDbTransaction transaction,
        string name)
    {
        return connection.ExecuteScalarAsync<int>(@"INSERT INTO api_resources(name) VALUES(@name) RETURNING id",
            new {name},
            transaction);
    }
    ...
    private static Task<int> GetExistingApiResourceIdByName(IDbConnection connection, IDbTransaction transaction,
        string name)
    {
        return connection.ExecuteScalarAsync<int>(@"SELECT id FROM api_resources WHERE api_resources.name =@name",
            new {name},
            transaction);
    }
    ...
    private static Task DeleteApiScope(IDbConnection connection, IDbTransaction transaction,
        int id)
    {
        return connection.ExecuteAsync(@"DELETE FROM api_scopes WHERE api_scopes.id = @id", new {id}, transaction);
    }
    ...
    {
        return connection.ExecuteAsync(
            @"UPDATE api_resources SET updated = current_timestamp WHERE api_resources.id = @id", new {id},
            transaction);
    }
}
