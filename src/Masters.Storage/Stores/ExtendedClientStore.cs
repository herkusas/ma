...
public class ExtendedClientStore : IExtendedClientStore
{
    ...
    private async Task Update(Client client, Client existingClient)
    {
        ...
        try
        {
            ...
            if (thereWasChanges)
                await UpdateClient(connection, transaction, existingClientId,client.ClientName, client.AccessTokenLifetime);
    ...
    private static Task<int> GetExistingClientId(IDbConnection connection, IDbTransaction transaction,
        string clientId) =>
        connection.ExecuteScalarAsync<int>(@"SELECT id FROM clients WHERE clients.client_id =@clientId", new {clientId},
            transaction);
    ...
    private static Task UpdateClient(IDbConnection connection, IDbTransaction transaction,
        ...
        connection.ExecuteAsync(@"UPDATE clients SET updated = current_timestamp, client_name = @name, access_token_lifetime = @clientAccessTokenLifetime WHERE clients.id = @id", new {id,name, clientAccessTokenLifetime},
            transaction);
}