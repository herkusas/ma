...
public class CertificateAuthenticationMiddleware
{
    ...
    public async Task Invoke(HttpContext httpContext)
    {
        if (httpContext.User.Identity is {IsAuthenticated: true})
        {
            ...
            {
                var certificateAuthenticateResult =
                    await httpContext.AuthenticateAsync(CertificateAuthenticationDefaults.AuthenticationScheme);
                ...
                var certificateHash =
                    Base64UrlTextEncoder.Encode(clientCertificate!.GetCertHash(HashAlgorithmName.SHA256));

                var x509CertificateSha256Thumbprint = JsonSerializer
                    .Deserialize<Dictionary<string, string>>(proofOfPossessionClaim, new JsonSerializerOptions())!
