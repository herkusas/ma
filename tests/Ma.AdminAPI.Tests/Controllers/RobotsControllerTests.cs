...
public class RobotsControllerTests
{
    ...
    private readonly RobotRecord _robot = new("robot_id", RobotName, s_scopes, new List<string> {"test_thumbprint"},
        3600);
    ...
    public async void Save_RequestIsValidRobotAlreadyExist_OkResultReturned()
    {
        ...
        //assert 
        result.Should().BeOfType<OkObjectResult>().Which.Value.Should().BeOfType<RobotRecord>().Which.Name.Should()
            .Be(RobotName);
    }
    ...
    public async void Save_RequestIsValidRobotDoesNotExist_CreatedActionReturned()
    {
        ...
        //assert 
        result.Should().BeOfType<CreatedAtActionResult>().Which.Value.Should().BeOfType<RobotRecord>().Which.Name
            .Should().Be(RobotName);
    }
}
